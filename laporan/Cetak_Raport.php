<?php
include('koneksi.php') OR exit('No direct script access allowed');
...
class Cetak_raport extends CI_Controller {
    function __construct() {
        ...
        $get_tasm = $this->db->query("SELECT tahun, nama_gr, nip, tgl_raport FROM tahun WHERE aktif = 'Y'")->row_array();
        $this->d['tasm'] = $get_tasm['thn'];
        $this->d['ta'] = substr($get_tasm['thn'],0,4);

        $this->d['wk'] = $this->session->userdata('app_rapot_walikelas');   
    }

    public function sampul1($id_siswa) {
        $d['ds'] = $this->db->query("SELECT nama, nis FROM murid WHERE id = '$nis'")->row_array();

        $this->load->view('cetak_sampul1', $d);

    }

    public function sampul2($nis) {
        $d = null;

        $this->load->view('cetak_sampul2', $d);
    }

    public function sampul4($nis) {
    ...
    public function pribadi($nis,$tasm) {
    ...
    public function cetak($id_siswa,$tasm) {
        ...
        $ambil_np_submp = $this->db->query("SELECT 
                                    ...
                                    FROM nilai a
                                    INNER JOIN mapel b ON a.id_guru_mapel = b.id
                                    INNER JOIN m_mapel c ON b.id_mapel = c.id
